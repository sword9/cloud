...
import com.hochan.sqlite.Filechose.FileListItem.ModeCallback;
import com.hochan.sqlite.Filechose.FileOperationHelper.IOperationProgressListener;
import com.hochan.sqlite.Filechose.FileSortHelper.SortMethod;
import com.hochan.sqlite.Filechose.FileViewActivity.SelectFilesCallback;
import com.hochan.sqlite.R;
import com.hochan.sqlite.Filechose.TextInputDialog.OnFinishListener;
...
public class FileViewInteractionHub implements IOperationProgressListener {
    ...
    public void onOperationSend() {
        ...
        for (FileInfo f : selectedFileList) {
            ...
            myFileInfo fileInfo = new myFileInfo(f.filePath, f.fileName, 0, 0, false, f.fileSize);
            Intent intent = new Intent(mContext, FileUploadUi.class).setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);;
            intent.setAction("UPLOAD_ACTION_START");
            intent.putExtra("fileinfo",fileInfo);

            /*mContext.sendBroadcast(intent)*/
            mContext.startActivity(intent);
            Log.d("file name",f.fileName);
            Log.d("file size",String.valueOf(f.fileSize));
            Log.d("file path",f.filePath);
            FileExplorerTabActivity.isFinish = true;
            //Intent intent1 = new Intent(mContext, File);

        }
        Log.d("form here on", "haha");
